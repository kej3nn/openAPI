package egovframework.common.base.mapper;
...
public class BaseMapper {
    ...
    public Paging search(String id, Object params, int page, int rows, int mode, boolean announce, boolean totality) {
        ...
        if (announce) {
            // 상단 데이터를 검색한다.
            data.addAll((List<?>) sqlSession.selectList(id + SUFFIX_UPPER, params));
        }
        ...
        if (count > 0) {
            ...
            if (PAGING_SCROLL == mode) {
                // 데이터를 검색한다.
                //data.addAll((List<?>) sqlSession.selectList(id, params, index * rows, rows));
            }
            // 매뉴얼 페이징 모드인 경우
            else {
                ...
                // 데이터를 검색한다.
                data.addAll((List<?>) sqlSession.selectList(id, parameters));
            }
        }
     ...
     * @return 조회결과
     */
    public Object selectList(String id, Object params) {
        return sqlSession.selectList(id, params);
    }
     ...
     * @return 조회결과
     *//*
    public Object selectList(String id, Object params, Object record) {
        return sqlSession.selectList(id, params, record);
    }
    */
