/*
 * @(#)PortalOpenInfLinkServiceImpl.java 1.0 2015/06/15
 * 
...
/**
 * 공공데이터 링크 서비스를 관리하는 서비스 클래스이다.
 * 
 * @author 김은삼
 * @version 1.0 2015/06/15
 */
@Service("ggportalOpenInfLinkService")
public class PortalOpenInfLinkServiceImpl extends PortalOpenInfSrvServiceImpl implements PortalOpenInfLinkService {
    /**
     * 공공데이터 링크 서비스를 관리하는 DAO
     */
    @Resource(name="ggportalOpenInfLinkDao")
    private PortalOpenInfLinkDao portalOpenInfLinkDao;
    ...
    private String getThumbnailPath(Record data) {
        ...
        if (!"".equals(data.getString("tmnlImgFile"))) {
            buffer.append(UtilString.getDownloadFolder("L", data.getString("seq")));
    ...
    public Record selectOpenInfLinkMeta(Params params) {
        // 공공데이터 링크 서비스 메타정보를 조회한다.
        Record meta = portalOpenInfLinkDao.selectOpenInfLinkMeta(params);
        try{
	        // 메타정보가 없는 경우
	        if (meta == null) {
    ...
    public List<?> searchOpenInfLinkData(Params params) {
        // 공공데이터 링크 서비스 데이터를 검색한다.
        return portalOpenInfLinkDao.searchOpenInfLinkData(params);
    }
    ...
    public Record selectOpenInfLinkDataCUD(Params params) {
        // 공공데이터 링크 서비스 데이터를 조회한다.
        Record link = portalOpenInfLinkDao.selectOpenInfLinkData(params);
        try{
	        // 데이터가 없는 경우
	        if (link == null) {
	        ...
	        portalOpenInfLinkDao.insertOpenInfLinkHist(params);
	        
	        // 공공데이터 링크 서비스 조회수를 수정한다.
	        portalOpenInfLinkDao.updateOpenInfLinkHits(params);
    ...
    public Record selectOpenInfLinkTmnl(Params params) {
        // // 공공데이터 링크 서비스 썸네일을 조회한다.
        // Record thumbnail = portalOpenInfLinkDao.selectOpenInfLinkTmnl(params);
        // 
