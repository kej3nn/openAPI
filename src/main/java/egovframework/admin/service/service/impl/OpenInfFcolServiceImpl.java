...
@Service("OpenInfFcolService")
public class OpenInfFcolServiceImpl extends OpenInfSrvServiceImpl implements OpenInfSrvService {

    private static final Logger logger = Logger.getLogger(OpenInfFcolServiceImpl.class);

    @Resource(name = "OpenInfFcolDAO")
    private OpenInfFcolDAO openInfColDAO;
    ...
    public int openInfColCUD(@NonNull OpenInfSrv openInfSrv, ArrayList<?> list) {
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                if (!((OpenInfFcol) list.get(i)).getStatus().equals("D")) {
                    result = openInfColDAO.mergeIntoCol((OpenInfFcol) list.get(i));
                } else if (((OpenInfFcol) list.get(i)).getStatus().equals("D")) {
                    result = openInfColDAO.deleteFcol((OpenInfFcol) list.get(i));
                }
            }
    ...
    public Map<String, Object> selectOpenInfColViewPopInfo(OpenInfSrv openInfSrv) {
        Map<String, Object> map = new HashMap<String, Object>();
        try {
            List<OpenInfLcol> resultList = openInfColDAO.selectOpenInfColList(openInfSrv);
