...
@Service("OpenInfLcolService")
public class OpenInfLcolServiceImpl extends OpenInfSrvServiceImpl implements OpenInfSrvService {

    private static final Logger logger = Logger.getLogger(OpenInfLcolServiceImpl.class);

    @Resource(name = "OpenInfLcolDAO")
    private OpenInfLcolDAO openInfColDAO;
    ...
    public Map<String, Object> selectOpenInfColList(OpenInfSrv openInfSrv) {
        ...
        try {
            List<OpenInfLcol> result = openInfColDAO.selectOpenInfColList(openInfSrv);
    ...
    public int openInfColCUD(@NonNull OpenInfSrv openInfSrv, ArrayList<?> list) {
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                if (!((OpenInfLcol) list.get(i)).getStatus().equals("D")) {
                    result = openInfColDAO.mergeIntoCol((OpenInfLcol) list.get(i));
                }
            }
    ...
    public Map<String, Object> selectOpenInfColViewPopInfo(OpenInfSrv openInfSrv) {
        Map<String, Object> map = new HashMap<String, Object>();
        try {
            List<OpenInfLcol> resultList = openInfColDAO.selectOpenInfColPopList(openInfSrv);
    ...
    public int updateTmnlImgFile(ArrayList<?> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            result = openInfColDAO.updateTmnlImgFile((OpenInfLcol) list.get(i));
        }
        return result;
    }
    ...
    public Map<String, Object> selectOpenInfColInfo(OpenInfSrv openInfSrv, int linkSeq) {
        OpenInfLcol param = (OpenInfLcol) openInfSrv;
        param.setLinkSeq(linkSeq);
        List<OpenInfLcol> resultList = openInfColDAO.selectOpenInfColInfo(param);
        Map<String, Object> map = new HashMap<String, Object>();
