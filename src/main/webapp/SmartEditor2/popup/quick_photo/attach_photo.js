...
//    	sUploadURL= 'http://test.naver.com/popup/quick_photo/FileUploader_html5.php'; 	//upload URL
    	sUploadURL = location.href.replace(/\/[^\/]*$/, '') + '/file_uploader_html5.jsp';
    ...
    function makeArrayFromString(sResString){
    	...
    	// 스마트에디터에서 get방식으로 못넘어와서 앞단에서 fileDir로 넘겨서 변환.. 
    	for (var key in aResult[0]) {
    		if ( key == "sFileURL" ) {
    			var aFileDirGubun = ["todayDir", "fileNm", "fileExt"];
    			var aFileDir = aResult[0]["sFileDir"].split('/');
    			for ( var i=0; i < aFileDir.length; i++ ) {
    				aFileDir[i] = aFileDirGubun[i] + "=" + aFileDir[i];
    			}
    			aResult[0][key] += "?" + aFileDir.join('&');
    		}
    	}
