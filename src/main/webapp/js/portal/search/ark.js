...
var arkDownId = "ark_down";						// 자동완성 down 이미지 <div> 의 id을 설정한다var totalRwCount = 0;							// 후방 검색 전체 개수
//var target = "";								// ARK 웹서버 설정파일의 목록에 있는 추천어 서비스 대상을 지정한다.
var target = "";								// ARK 웹서버 설정파일의 목록에 있는 추천어 서비스 대상을 지정한다.
var charset = "utf-8";							// 인코딩 설정 (인코딩이 utf-8이 아닐 경우 8859_1 로 설정해야함)
var datatype = "json";							// 반환받을 Data의 타입을 설정. XML 과 JSON이 가능 (xml | json)
var arkPath = "/search/images/ark/";						// 자동완성 경로
var transURL =  "/portal/search/getArk.do";		// trans 페이지의 URL을 설정한다.
var tempQuery = "";
 ...
 */
var browser = "";
if (navigator.userAgent.match("MSIE") == 'MSIE' || ( navigator.userAgent.match("Trident") && (navigator.appName=='Netscape'))) {
	browser = "IE";
...
var browserVersion = navigator.appVersion;			// 웹브라우져의 버전
var offset = null;
var offsetTop = 0;
var offsetLeft = 0;
...
var arkImgTop = 20;								// 자동완성 화살표 이미지의 상단에서 위치 값을 설정한다.
var arkImgLeft = -25;							// 자동완성 화살표 이미지의 왼쪽에서 위치 값을 설정한다.
...
$(document).ready(function() {
	...
	$("#arkb").click(
		function() {
			if($("#arkb").attr('class').match('on') == 'on'){
				hideArk();
			}else{
				if ($(queryId).val() != "") {
					requestArkJson($(queryId).val());
...
function drawArk() {
	...
	// 자동완성 접기/펼침 이미지 생성
	str = "<div id=\"" + arkDownId + "\" style=\"position:absolute; display:block; cursor:pointer;\"><img id=\"" + imgDownId + "\" src=\"" + arkPath + "/arrow_auto.gif\" alt=\"자동완성펼치기\"></div>";
	str += "<div id=\"" + arkUpId + "\" style=\"position:absolute; display:none; cursor:pointer;\"><img id=\"" + imgUpId + "\" src=\"" + arkPath + "/arrow_auto2.gif\" alt=\"자동완성접기\" ></div>";
	
	// 툴팁 이미지 생성
	//str += "<div id=\"tooltip01\" style=\"position:absolute; display:none; cursor:pointer; z-index:999999;\"><img id=\"tooltipIcon01\" src=\"" + arkPath +"/tooltip_01.gif\"/></div>";
...
function requestArkJson(query) {
	...
	$.ajax({
		...
		success: function(datahtml) {
			...
			$.each(dataJson.result, function(i, result) {
				...
				if (totalCount > 0) {
					...
					$.each(result.items, function(num,item){
						...
						str += "<li id=\"bg" + num + "\" onmouseover=\"onMouseOverKeyword(" + num + ");\"";
						str += " onmouseout=\"onMouseOutKeyword(" + num + ");\"><a href=\"javascript:onClickKeyword(" + num + ");\">"  + item.hkeyword;
						str += " <span id=\"f" + num + "\" style=display:none;>" + item.keyword + "</span></a></li>";
...
function setArkFooter() {
	...
	if (isArk) {
		...
		if ($(queryId).val() != "" && isListShow) {
			...
			//$("body").append("<span style=\"display:none;\"><div id=\"tooltip02\" style=\"position:absolute;\"><img id=\"tooltipIcon02\" src=\"" + arkPath +"/tooltip_02.gif\"/></div></span>");
		} else {
