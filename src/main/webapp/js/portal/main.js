...
function renderAddIcon(gubun, date) {
	...
	if(!com.wise.util.isBlank(day.find("span").text())) {
		day.find("span").closest("a").attr("href", "javascript:;");
	}
}
...
function afterSelectAssmLiveStat(datas) {
	...
	if ( datas.length > 0 ) {
		...
		var speakTime = sptSubj[0].substring(_.indexOf(sptSubj[0], '['), _.indexOf(sptSubj[0], ']')+1);
		var speakDesc = sptSubj[0].substr(0, _.lastIndexOf(sptSubj[0], '[')-1) + " " + firData.xDesc;
		
		$("#live_speak_desc").text(com.wise.util.ellipsis(speakTime + " " + speakDesc, 60));
		
		$("#live_confer_no").text(sptSubj[0]+")" || "");
		$("#live_confer_subj").text("제"+sptSubj[0]);
...
function fnLiveSubjChange(billId) {
	...
	$("#live_confer_no").text(sptSubj[0]+")" || "");
	$("#live_confer_subj").text(com.wise.util.ellipsis("제"+(sptSubj[0]||"") || "", 23));
...
function initSlider() {
	...
	// 공지사항 슬라이더
	SLIDER.notice = $("#notice-div-sect").bxSlider({
		mode : 'vertical',
	 	speed : 500,
	 	pager : false,
	 	moveSlider : 1,
...
function initSliderEvent() {
	// 공지사항 슬라이더 이전
	$(".notice_prev").bind("click", function() {
		SLIDER.notice.goToPrevSlide();
		return false;
	}).bind("keydown", function(event) {
		if ( event.which == 13 ) {
			SLIDER.notice.goToPrevSlide();
	...
	// 공지사항 슬라이더 다음
	$(".notice_next").bind("click", function() {
		SLIDER.notice.goToNextSlide();
		return false;
	}).bind("keydown", function(event) {
		if ( event.which == 13 ) {
			SLIDER.notice.goToNextSlide();
