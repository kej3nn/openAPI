/**
 * 의정활동별 공개 - 위원회 구성/계류법안 - 위원회 회의록 스크립트 파일이다.
 * 
 * @author JHKIM
 ...
 * @returns
 */
function searchCmpReport(page) {
	page = page || 1;
	doSearch({
		url : "/portal/bpm/cmp/searchCmpReport.do",
		page : page,
		before : beforeSearchCmpReport,
		after : afterSearchCmpReport,
 ...
 * 목록 리스트 페이징 조회 전처리
 */
function beforeSearchCmpReport(options) {
	var form = $("#reportForm");
 ...
 * 목록 리스트 페이징 조회 후처리
 */
function afterSearchCmpReport(datas) {
	var row = null,
		data = null,
		list = $("#report-result-sect"),
		item = 
			"<tr>" +
			"	<td class=\"rownum\"></td>" +
			"	<td class=\"daeNum\"></td>" +
			...
			"	<td><span class=\"commName\"></span></td>" +
...
function bindCmpReportEvent() {
	...
	// 조회 엔터이벤트
	formObj.find("input").bind("keydown", function(event) {
		if ( event.which == 13 ) {
			searchCmpReport(1);
			return false;
		}
	...
	formObj.find("#btnDownload").bind("click", function() {
		...
		if ( com.wise.util.isBlank(data.toConfDate) )	searchParamCnt++;
		if ( com.wise.util.isBlank(data.className) )	searchParamCnt++;
		if ( com.wise.util.isBlank(data.title) )	searchParamCnt++;
		if ( com.wise.util.isBlank(data.commName) )	searchParamCnt++;
