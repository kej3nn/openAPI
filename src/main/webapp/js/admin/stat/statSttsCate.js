/*
 * @(#)statStddUi.js 1.0 2017/08/21
 * 
 * COPYRIGHT (C) 2013 WISEITECH CO., LTD. ALL RIGHTS RESERVED.
 */

/**
 * 관리자 통계표 분류 스크립트 파일이다
 *
 * @author 김정호
...
function doAction(sAction) {
	...
	var formObj = getTabFormObj("sttsCate-dtl-form");
	var sheet = window["sheet"];
	...
	switch(sAction) {                       
		case "search":
			...
			sheet.DoSearchPaging(com.wise.help.url("/admin/stat/statSttsCateList.do"), param);
			break;
		case "regForm" :
			var title = "신규등록"
			var id ="statReg";
		...
		case "dupChk" : 
			doPost({
                url:"/admin/stat/statSttsCateDupChk.do",
		...
		case "delete" :
			...
			doAjax({
				url : "/admin/stat/deleteStatSttsCate.do",
		...
		case "sttsCatePop" :
			...
			var url = com.wise.help.url("/admin/stat/popup/statSttsCatePop.do");
			var data = "?parentFormNm=sttsCate-dtl-form";
			wWidth ="660";                                                    
			wHeight ="530";   
		...
		case "orderSave" :
			saveSheetData({
                SheetId:"sheet",
                PageUrl:"/admin/stat/saveStatSttsCateOrder.do"
            }, {
            }, {
                AllSave:0
            });
...
function saveData(action) {
	var formObj = getTabFormObj("sttsCate-dtl-form");
	formObj.ajaxSubmit({
		url : com.wise.help.url("/admin/stat/saveStatSttsCate.do")
		, async : false
		, type : 'post'
		, dataType: 'json' 
...
function tabFunction(tab, data) {
	...
	//이미지 표시
	formObj.find("#cateImg").attr("src", "/admin/stat/selectThumbnail.do?" + "cateId=" + data.DATA.cateId);
}
/**
 * 트리 자식 데이터 로드
 */
...
function tabEvent(row){//탭 이벤트 실행  
	...
	var url =  com.wise.help.url('/admin/stat/statSttsCateDtl.do'); // Controller 호출 url 
...
function setTabButton() {
	...
	formObj.find("button[name=sttsCate_pop]").bind("click", function() {
		//분류 팝업
		doAction("sttsCatePop");
	});
	formObj.find("button[name=sttsCate_init]").bind("click", function() {
		//분류 초기화
		doAction("sttsCateInit");
	});
	formObj.find("button[name=sttsCate_dup]").bind("click", function() {
		//분류ID 중복체크
		doAction("dupChk");
	});
...
function saveValidation(action) {
	...
	var formObj = objTab.find("form[name=sttsCate-dtl-form]");
	...
	if ( com.wise.util.isNull(formObj.find("input[name=parCateId]").val()) ) {
		alert("상위분류를 선택하세요.");
		formObj.find("button[name=sttsCate_pop]").focus();
		return false;
	}
	/* 필수체크 제거
