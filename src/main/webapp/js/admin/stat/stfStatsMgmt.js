/*
 * @(#)stfStatsMgmt.js 1.0 2019/09/09
 */

/**
 * 관리자에서 내부직원용 통계데이터 메타 관리하는 스크립트
 *
 * @author 
...
function doAction(sAction) {
	var mainFormObj = $("form[name=stfStatsMgmtMainForm]");
	var formObj = getTabFormObj("stfStatsMgmt-dtl-form");
	...
	switch(sAction) {                       
		case "search":
			...
			sheet.DoSearchPaging(com.wise.help.url("/admin/stat/selectStfStatsMgmtListPaging.do"), param);
			break;
		case "regForm" :
			var title = "신규등록"
			var id ="siteMapReg";
			...
			break;
		case "catePop" :
			var url = com.wise.help.url("/admin/stat/popup/naSetCateSearchPop.do");
			var data = "?parentFormNm=stfStatsMgmtMainForm";
			openIframeStatPop("iframePopUp", url+data, 660, 530);
			break;
		case "naSetCatePop" :
			var url = com.wise.help.url("/admin/stat/popup/naSetCatePop.do");
			var data = "?parentFormNm=stfStatsMgmt-dtl-form";
			wWidth ="660";                                                    
			wHeight ="530";   
		...
		case "dupChk" : 
			doPost({
                url:"/admin/stat/stfStatsMgmtDupChk.do",
		...
		case "statPreviewPop" :	//통계표 미리보기 팝업
			window.open(com.wise.help.url("/portal/stat/selectServicePage.do") +"/"+ formObj.find("input[name=statblId]").val() , "list", "fullscreen=no, width=1152, height=768, scrollbars=yes");
			break;
...
function saveData(action) {
	var formObj = getTabFormObj("stfStatsMgmt-dtl-form");
	formObj.ajaxSubmit({
		url : com.wise.help.url("/admin/stat/saveStfStatsMgmt.do")
		, async : false
		, type : 'post'
		, dataType: 'json' 
...
function beforeNaDataSiteMapDupChk(options) {
	var formObj = getTabFormObj("stfStatsMgmt-dtl-form");
	var statblId = formObj.find("input[name=statblId]").val().trim().toUpperCase();
 var data = {
     "statblId" : statblId
 };
 if ( com.wise.util.isBlank(data.statblId) ) {
 	alert("서비스맵ID를 입력하세요.");
 	formObj.find("input[name=statblId]").focus();
     return null;
 }
 
 formObj.find("input[name=statblId]").val(statblId);	//소문자로 입력되어있을경우 대문자로 변경해서 보여줌
 
 if ( !com.wise.util.isAlphaNumeric(data.statblId) || !com.wise.util.isLength(data.statblId, 1, 10) ) {
 	alert("영문, 숫자 10자리 이내로 입력하세요.");
 	formObj.find("input[name=statblId]").val("").focus();
     return null;
 }
...
function tabFunction(tab, data) {
	var formObj = getTabFormObj("stfStatsMgmt-dtl-form");
	formObj.find("button[name=stfStatsMgmtInfoId_dup]").hide();			//중복확인 버튼 숨김(수정못함)
	formObj.find("input[name=dupChk]").val("Y");				//중복체크 Y로
	formObj.find("input[name=statblId]").attr("readonly", true);	//분류ID 수정 못하도록 변경
	
	//이미지 표시
	formObj.find("#tmnlImgFileImg").attr("src", "/admin/stat/selectThumbnail.do?" + "statblId=" + data.DATA.statblId );

	var obj1 = formObj.find("input[name=statblNm]");
...
function afterNaDataSiteMapDupChk(data) {
	...
	if ( data.dupCnt > 0 ) {
		...
		formObj.find("input[name=statblId]").attr("readonly", true);	//중복체크 완료 후 단위ID변경 불가
	}
}
...
function regUserFunction() {
	var formObj = getTabFormObj("stfStatsMgmt-dtl-form");
	formObj.find("input[name=statblId]").attr("readonly", false).removeClass("readonly");
	formObj.find("input[name=useYn][value=Y]").prop("checked", true);
}
//상세 탭 이벤트
function tabEvent(row){//탭 이벤트 실행  
	var title = sheet.GetCellValue(row, "statblNm");//탭 제목
	var id = sheet.GetCellValue(row, "statblId");//탭 id(유일한key))
	openTab.SetTabData(sheet.GetRowJson(row));//db data 조회시 조건 data
	var url =  com.wise.help.url('/admin/stat/stfStatsMgmtDtl.do'); // Controller 호출 url 
...
function byteCheck(val, name){
	...
	var formObj = objTab.find("form[name=stfStatsMgmt-dtl-form]");
...
function setTabButton() {
	...
	formObj.find("button[name=stfStatsMgmtInfoId_dup]").bind("click", function() {
		//사이트맵ID 중복체크
		doAction("dupChk");
	});
	formObj.find("input[name=statblId]").keypress(function(e) {
		  if(e.which == 13) {
			  doAction('dupChk');   
			  return false;        
		  }
	...
	//통계데이터명
	formObj.find("input[name=statblNm]").bind("keyup", function(event) {
		var obj = formObj.find("input[name=statblNm]");
...
createIBSheet2(document.getElementById("sheet"),"sheet", "100%", "350px");	// InfoSetMain Sheet
	...
	var gridTitle = "NO|통계표ID|통계표명|분류|담당부서|이용허락조건|개방일";
	...
	with(sheet){
	    ...
	    var cols = [
	                ...
	                ,{Type:"Text",		SaveName:"statblId",			Width:50,	Align:"Center",	 Edit:false}
	                ,{Type:"Text",		SaveName:"statblNm",			Width:200,	Align:"Left",	 Edit:false}
...
function saveValidation(action) {
	...
	if ( com.wise.util.isNull(formObj.find("input[name=statblNm]").val()) ) {
		alert("공공데이터명을 입력하세요.");
		formObj.find("input[name=statblNm]").focus();
		return false;
	} else if ( !com.wise.util.isLength(formObj.find("input[name=statblNm]").val(), 1, 150) ) {
