...
function doAction(sAction) {
	var mainFormObj = $("form[name=docInfMainForm]");
	var formObj = getTabFormObj("docInfCate-dtl-form");
	var sheet = window["sheet"];
	...
	switch(sAction) {                       
		case "search":
			...
			sheet.DoSearchPaging(com.wise.help.url("/admin/infs/cate/docInfCateList.do"), param);
			break;
		case "regForm" :
			var title = "신규등록"
			var id ="statReg";
		...
		case "dupChk" : 
			doPost({
                url:"/admin/infs/cate/docInfCateDupChk.do",
		...
		case "delete" :
			...
			doAjax({
				url : "/admin/infs/cate/deleteDocInfCate.do",
		...
		case "docInfCatePop" :
			...
			var url = com.wise.help.url("/admin/infs/cate/popup/docInfCatePop.do");
			var data = "?parentFormNm=docInfCate-dtl-form";
			wWidth ="660";                                                    
			wHeight ="530";   
		...
		case "orderSave" :
			saveSheetData({
                SheetId:"sheet",
                PageUrl:"/admin/infs/cate/saveDocInfCateOrder.do"
            }, {
            }, {
                AllSave:0
            });
...
function saveData(action) {
	var formObj = getTabFormObj("docInfCate-dtl-form");
	formObj.ajaxSubmit({
		url : com.wise.help.url("/admin/infs/cate/saveDocInfCate.do")
		, async : false
		, type : 'post'
		, dataType: 'json' 
...
function tabFunction(tab, data) {
	...
	//이미지 표시
	formObj.find("#cateImg").attr("src", "/admin/infs/cate/selectDocInfCateThumbnail.do?" + "cateId=" + data.DATA.cateId);
}
/**
 * 트리 자식 데이터 로드
 */
...
function tabEvent(row){//탭 이벤트 실행  
	...
	var url =  com.wise.help.url('/admin/infs/cate/docInfCateDtl.do'); // Controller 호출 url 
...
function setTabButton() {
	...
	formObj.find("button[name=docInfCate_pop]").bind("click", function() {
		//분류 팝업
		doAction("docInfCatePop");
	});
	formObj.find("button[name=docInfCate_init]").bind("click", function() {
		//분류 초기화
		doAction("docInfCateInit");
	});
	formObj.find("button[name=docInfCate_dup]").bind("click", function() {
		//분류ID 중복체크
		doAction("dupChk");
	});
...
function saveValidation(action) {
	...
	var formObj = objTab.find("form[name=docInfCate-dtl-form]");
	...
	if ( com.wise.util.isNull(formObj.find("input[name=parCateId]").val()) ) {
		alert("상위분류를 선택하세요.");
		formObj.find("button[name=docInfCate_pop]").focus();
		return false;
	}
	/* 필수체크 제거
