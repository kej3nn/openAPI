...
function doAction(sAction) {
	var mainFormObj = $("form[name=infSetMainForm]");
	var formObj = getTabFormObj("infSetCate-dtl-form");
	var sheet = window["sheet"];
	...
	switch(sAction) {                       
		case "search":
			...
			sheet.DoSearchPaging(com.wise.help.url("/admin/infs/cate/infSetCateList.do"), param);
			break;
		case "regForm" :
			var title = "신규등록"
			var id ="statReg";
		...
		case "dupChk" : 
			doPost({
                url:"/admin/infs/cate/infSetCateDupChk.do",
		...
		case "delete" :
			...
			doAjax({
				url : "/admin/infs/cate/deleteInfSetCate.do",
		...
		case "infSetCatePop" :
			...
			var url = com.wise.help.url("/admin/infs/cate/popup/infSetCatePop.do");
			var data = "?parentFormNm=infSetCate-dtl-form";
			wWidth ="660";                                                    
			wHeight ="530";   
		...
		case "orderSave" :
			saveSheetData({
                SheetId:"sheet",
                PageUrl:"/admin/infs/cate/saveInfSetCateOrder.do"
            }, {
            }, {
                AllSave:0
            });
...
function saveData(action) {
	var formObj = getTabFormObj("infSetCate-dtl-form");
	formObj.ajaxSubmit({
		url : com.wise.help.url("/admin/infs/cate/saveInfSetCate.do")
		, async : false
		, type : 'post'
		, dataType: 'json' 
...
function tabFunction(tab, data) {
	...
	//이미지 표시
	formObj.find("#cateImg").attr("src", "/admin/infs/cate/selectThumbnail.do?" + "cateId=" + data.DATA.cateId);
}
/**
 * 트리 자식 데이터 로드
 */
...
function tabEvent(row){//탭 이벤트 실행  
	...
	var url =  com.wise.help.url('/admin/infs/cate/infSetCateDtl.do'); // Controller 호출 url 
...
function setTabButton() {
	...
	formObj.find("button[name=infSetCate_pop]").bind("click", function() {
		//분류 팝업
		doAction("infSetCatePop");
	});
	formObj.find("button[name=infSetCate_init]").bind("click", function() {
		//분류 초기화
		doAction("infSetCateInit");
	});
	formObj.find("button[name=infSetCate_dup]").bind("click", function() {
		//분류ID 중복체크
		doAction("dupChk");
	});
...
function saveValidation(action) {
	...
	var formObj = objTab.find("form[name=infSetCate-dtl-form]");
	...
	if ( com.wise.util.isNull(formObj.find("input[name=parCateId]").val()) ) {
		alert("상위분류를 선택하세요.");
		formObj.find("button[name=infSetCate_pop]").focus();
		return false;
	}
	/* 필수체크 제거
