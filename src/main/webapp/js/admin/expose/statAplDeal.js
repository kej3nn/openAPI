/*
 * @(#)statAplDeal.js 1.0 2019/08/23
 */

/**
 * 관리자 메뉴 청구방법별 현황 스크립트 파일이다
 *
 * @author 최성빈
...
function doAction(sAction) {
	...
	switch(sAction) {                       
		case "search":
			...
			sheet.DoSearchPaging(com.wise.help.url("/admin/expose/statAplDeal.do"), { Param : $("form[name=aplDealForm]").serialize() });
			break;
		case "excel":
			sheet.Down2Excel({FileName:'청구서 처리현황.xls',SheetName:'sheet'});
			break;
...
function fn_print() {
	...
	formPrint.find("input[name=title]").val("청구서 처리현황");
	...
	formPrint.attr("action", com.wise.help.url("/admin/expose/infoPrintPage/printNewStatAplDeal.do"));
...
function goOpnAplListPop(instCd, saveNm) {
	...
	$("input[name=opbYnp]").val("");
	$("input[name=prg]").val("");
	$("input[name=trsf]").val("");
	 
	var form = $("form[name=aplDealForm]");
    ...
    //SaveName 따라 파라미터 세팅
	if(saveNm == "totSum2")   $("input[name=opbYnp]").val("tot");
	else if(saveNm == "sum1") $("input[name=opbYnp]").val("0");
	else if(saveNm == "sum2") $("input[name=opbYnp]").val("1");
	else if(saveNm == "sum3") $("input[name=opbYnp]").val("2");
	else if(saveNm == "sum4") $("input[name=prg]").val("opb_n");
	else if(saveNm == "sum5") $("input[name=trsf]").val("y");
