/*
 * @(#)statAplDcsDt.js 1.0 2019/08/23
 */

/**
 * 관리자 메뉴 공개여부 결정기간별 현황 스크립트 파일이다
 *
 * @author 최성빈
...
function doAction(sAction) {
	...
	switch(sAction) {                       
		case "search":
			...
			sheet.DoSearchPaging(com.wise.help.url("/admin/expose/statAplDcsDt.do"), { Param : $("form[name=aplDcsDtForm]").serialize() });
			break;
		case "excel":
			sheet.Down2Excel({FileName:'결정기간별 현황.xls',SheetName:'sheet'});
			break;
...
function fn_print() {
	...
	formPrint.find("input[name=title]").val("결정기간별 현황");
	...
	formPrint.attr("action", com.wise.help.url("/admin/expose/infoPrintPage/printNewStatAplDcsDt.do"));
...
function goOpnAplListPop(instCd, saveNm) {
	...
	var form = $("form[name=aplDcsDtForm]");
  ...
  //SaveName 따라 파라미터 세팅
	if(saveNm == "totSum1"){
		$("input[name=fromDt]").val("1");
		$("input[name=toDt]").val("10000");
	}else if(saveNm == "sum1"){
		$("input[name=fromDt]").val("1");
		$("input[name=toDt]").val("1");
	}else if(saveNm == "sum2"){
		$("input[name=fromDt]").val("2");
		$("input[name=toDt]").val("3");
	}else if(saveNm == "sum3"){
		$("input[name=fromDt]").val("4");
		$("input[name=toDt]").val("5");
	}else if(saveNm == "sum4"){
		$("input[name=fromDt]").val("6");
		$("input[name=toDt]").val("10");
	}else if(saveNm == "sum5"){
		$("input[name=fromDt]").val("11");
		$("input[name=toDt]").val("20");
	}else if(saveNm == "sum6"){
		$("input[name=fromDt]").val("21");
		$("input[name=toDt]").val("10000");
	}else if (saveNm == "instNm"){
		$("input[name=fromDt]").val("1");
		$("input[name=toDt]").val("10000");
	}
