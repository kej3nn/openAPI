...
function tabFunction(tab, data) {
	...
	if(dataDtl.opbYn != null){
		...
		tab.ContentObj.find("label[name=opbCn]").text(dataDtl.opbCn);
	}else{
	...
	if(dataDtl.imdFlNm != null){
		...
		tab.ContentObj.find("div[name=imdFileArea] > a, label[name=imdflNm] > a").bind("click", {
            param : "?fileNm=" + imdFlNm + "&filePath=" + imdFlPh
        }, function(event) {
            // 파일을 다운로드한다.
	...
	if(dataDtl.nonExt != null){
		...
		tab.ContentObj.find("label[name=nonExt]").text(dataDtl.nonExt);
	}else{
	...
	if(dataDtl.nonFlNm != null){
		...
		tab.ContentObj.find("div[id=nonFileArea] > a, label[id=nonFlNm] > a").bind("click", {
            param : "?fileNm=" + nonFlNm + "&filePath=" + nonFlPh
        }, function(event) {
            // 파일을 다운로드한다.
            downloadFile(event.data.param);
            return false;
	...
	if(dataDtl.clsdAttchFlNm != null){
		...
		tab.ContentObj.find("div[name=clsdFileArea] > a, label[name=clsdAttchFlNm] > a").bind("click", {
            param : "?fileNm=" + clsdAttchFlNm + "&filePath=" + clsdAttchFlPhNm
        }, function(event) {
            // 파일을 다운로드한다.
            downloadFile(event.data.param);
            return false;
	...
	if(dataDtl.opbFomVal != null){
		tab.ContentObj.find("input:radio[name=opb_fom]:input[value="+dataDtl.opbFomVal+"]").prop("checked", true);
		if(dataDtl.opbFomVal == "05") tab.ContentObj.find("input[name=opb_fom_etc]").show();
	}else{
		$('input:radio[name=opb_fom]').eq(0).prop("checked", true);
	}
	if(dataDtl.opbFomNm != null){
	...
	var aplTakMthEtc = dataDtl.aplTakMthEtc;
	if(aplTakMthEtc == null || aplTakMthEtc == undefined) aplTakMthEtc = "";	
	var etcHtml = "<input type='text' name='give_mth_etc' maxlength='50' size='25' value='"+aplTakMthEtc+"' style='display:none;'/>";
	tab.ContentObj.find("div[name=giveMthArea]").append(etcHtml);
	if(dataDtl.aplTakMth != null){
		tab.ContentObj.find("input:radio[name=give_mth]:input[value="+dataDtl.aplTakMth+"]").prop("checked", true);
		if(dataDtl.aplTakMth == "05") tab.ContentObj.find("input[name=give_mth_etc]").show();
	}else{
		$('input:radio[name=give_mth]').eq(0).prop("checked", true);
	}
	if(dataDtl.aplTakMthNm != null){
	...
	if(dataDtl.trdOpnFlNm != null){
		...
		tab.ContentObj.find("div[name=fileArea] > a, label[name=thirdOpnFlnm] > a").bind("click", {
            param : "?fileNm=" + trdOpnFlNm + "&filePath=" + trdOpnFlPh
        }, function(event) {
            // 파일을 다운로드한다.
            downloadFile(event.data.param);
            return false;
	...
	if(dataDtl.dbrtInstFlNm != null){
		...
		tab.ContentObj.find("div[name=file1Area] > a, label[name=dbrtInstFlnm] > a").bind("click", {
            param : "?fileNm=" + dbrtInstFlNm + "&filePath=" + dbrtInstFlPh
        }, function(event) {
            // 파일을 다운로드한다.
            downloadFile(event.data.param);
            return false;
	...
	if(dataDtl.opbFlNm !=null){
		...
		tab.ContentObj.find("a[name=opbFlNm]").bind("click", {
            param : "?fileNm=" + opbFlNm + "&filePath=" + opbFlPh
        }, function(event) {
            // 파일을 다운로드한다.
            downloadFile(event.data.param);
            return false;
	...
	if(dataDtl.opbFlNm2 !=null){
		...
		tab.ContentObj.find("a[name=opbFlNm2]").bind("click", {
            param : "?fileNm=" + opbFlNm2 + "&filePath=" + opbFlPh2
        }, function(event) {
            // 파일을 다운로드한다.
            downloadFile(event.data.param);
            return false;
	...
	if(dataDtl.opbFlNm3 !=null){
		...
		tab.ContentObj.find("a[name=opbFlNm3]").bind("click", {
            param : "?fileNm=" + opbFlNm3 + "&filePath=" + opbFlPh3
        }, function(event) {
            // 파일을 다운로드한다.
            downloadFile(event.data.param);
            return false;
...
function setTabButton() {
	...
	formObj.find("textarea[name=clsd_rmk]").bind("keyup", function(event) {
		var obj = formObj.find("textarea[name=clsd_rmk]");
		textAreaLenChk(obj, 'len4', 500);
	});
...
function saveValidation(action) {
	...
	if ( formObj.find("input[name=opb_yn]:checked").val()=="1" ) {
		var cnt = 0;
		formObj.find("input[name=clsd_content]").each(function() {
			if(com.wise.util.isNull($(this).val())) cnt++;
		});
		formObj.find("input[name=clsd_reason]").each(function() {
			if(com.wise.util.isNull($(this).val())) cnt++;
		});
		
		if(cnt>0){
			alert("비공개내용 및 사유를 입력해주세요.\n추가 내용이 없으면 [삭제] 처리해주세요.");
			return false;
		}
		if (com.wise.util.isNull(formObj.find("select[name=clsd_rson_cd]").val())  ) {
			alert("비공개 사유를 선택해주세요.");
			formObj.find("select[name=clsd_rson_cd]").focus();
			return false;
		}
		if (com.wise.util.isNull(formObj.find("textarea[name=opb_cn]").val())  ) {
			alert("공개내용을 입력해주세요.");
...
function fn_clsdDiv(val){
	...
	if(val == "0"){ //** 정보공개 결정 ▶ 공개
		...
		formObj.find("th[name=clsdTd1]").hide();
		formObj.find("td[name=clsdTd2]").hide();
		
		formObj.find("tr[name=clsdDiv2]").hide();      	//비공개 사유
		formObj.find("th[name=clsdTd3]").hide();
		formObj.find("td[name=clsdTd4]").hide();
		...
		formObj.find("th[name=clsdTd1]").show();
		formObj.find("td[name=clsdTd2]").show();
		
		formObj.find("tr[name=clsdDiv2]").show();      	//비공개 사유
		formObj.find("th[name=clsdTd3]").show();
		formObj.find("td[name=clsdTd4]").show();
...
function fn_print() {
	...
	pfrm.target = 'popup';
	pfrm.action = "/admin/expose/infoPrintPage/printOpnAplPrc.do";
	pfrm.submit();
}
...
function fn_clsdAdd(){
	...
	clsdHtml += '비공개내용 : <input type="text" style="width:300px;" name="clsd_content" value="" maxlength="500" /> ';
	clsdHtml += '비공개사유 : <input type="text" style="width:300px;" name="clsd_reason" value="" maxlength="500" /> ';
	clsdHtml += '<button type="button" class="btn01" name="clsdDel" onclick="javascript:fn_clsdDel('+nextClsdCnt+');">삭제</button><br/>';
	clsdHtml += '</label>';
