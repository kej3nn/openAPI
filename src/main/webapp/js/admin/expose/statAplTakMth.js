/*
 * @(#)statAplTakMth.js 1.0 2019/08/23
 */
...
function bindEvent() {
	...
	$("button[name=btn_inquiry]").bind("click", function(event) {
		...
		var frm = document.aplTakMthForm;
		//============================================
...
function doAction(sAction) {
	...
	switch(sAction) {                       
		case "search":
			...
			sheet.DoSearchPaging(com.wise.help.url("/admin/expose/statAplTakMth.do"), { Param : $("form[name=aplTakMthForm]").serialize() });
			break;
		case "excel":
			sheet.Down2Excel({FileName:'청구방법별 현황.xls',SheetName:'sheet'});
			break;
...
function fn_print() {
	...
	formPrint.find("input[name=title]").val("청구방법별 현황");
	...
	formPrint.attr("action", com.wise.help.url("/admin/expose/infoPrintPage/printNewStatAplTakeMth.do"));
...
function goOpnAplListPop(instCd, saveNm) {
	...
	$("input[name=aplTakMth]").val("");
	 
	var form = $("form[name=aplTakMthForm]");
	...
	if(saveNm == "sum1")      $("input[name=aplTakMth]").val("01");
	else if(saveNm == "sum2") $("input[name=aplTakMth]").val("02");
	else if(saveNm == "sum3") $("input[name=aplTakMth]").val("03");
	else if(saveNm == "sum4") $("input[name=aplTakMth]").val("04");
	else if(saveNm == "sum5") $("input[name=aplTakMth]").val("05");
