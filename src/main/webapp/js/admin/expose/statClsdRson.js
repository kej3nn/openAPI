/*
 * @(#)statClsdRson.js 1.0 2019/08/23
 */

/**
 * 관리자 메뉴 비공개 사유별 현황 스크립트 파일이다
 *
 * @author 최성빈
...
function doAction(sAction) {
	...
	switch(sAction) {                       
		case "search":
			...
			sheet.DoSearchPaging(com.wise.help.url("/admin/expose/statClsdRson.do"), { Param : $("form[name=clsdRsonForm]").serialize() });
			break;
		case "excel":
			sheet.Down2Excel({FileName:'비공개 사유별 현황.xls',SheetName:'sheet'});
			break;
...
function fn_print() {
	...
	formPrint.find("input[name=title]").val("비공개 사유별 현황");
	...
	formPrint.attr("action", com.wise.help.url("/admin/expose/infoPrintPage/printNewStatClsdRson.do"));
...
function goOpnAplListPop(instCd, saveNm) {
	...
	$("input[name=clsdRsonCd]").val("");
	 
	var form = $("form[name=clsdRsonForm]");
    ...
    //SaveName 따라 파라미터 세팅
	if(saveNm == "totSum1")   $("input[name=clsdRsonCd]").val("00");
    else if(saveNm == "sum1") $("input[name=clsdRsonCd]").val("01");
	else if(saveNm == "sum2") $("input[name=clsdRsonCd]").val("02");
	else if(saveNm == "sum3") $("input[name=clsdRsonCd]").val("03");
	else if(saveNm == "sum4") $("input[name=clsdRsonCd]").val("04");
	else if(saveNm == "sum5") $("input[name=clsdRsonCd]").val("05");
	else if(saveNm == "sum6") $("input[name=clsdRsonCd]").val("06");
	else if(saveNm == "sum7") $("input[name=clsdRsonCd]").val("07");
	else if(saveNm == "sum8") $("input[name=clsdRsonCd]").val("08");
	else if(saveNm == "sum9") $("input[name=clsdRsonCd]").val("09");
...
function loadSheet() {
	...
	gridTitle +="|구분";
	gridTitle +="|비공개 건수";
	gridTitle +="|법령상의 비밀·비공개\n(법제9조제1항제1호)";
	gridTitle +="|국방 등 국익침해\n(제2호)";
	gridTitle +="|국민의 생명 등 공익침해\n(제3호)";
	gridTitle +="|재판관련 정보등\n(제4호)";
	gridTitle +="|공정한 업무수행 지장등\n(제5호)";
	gridTitle +="|개인 사생활 침해\n(제6호)       ";
	gridTitle +="|법인 등 영업상 비밀침해\n(제7호)";
	gridTitle +="|특정인의 이익·불이익\n(제8호)";
	gridTitle +="|기타";
