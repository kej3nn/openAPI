/*
 * @(#)statAplOpbFom.js 1.0 2019/08/23
 */

/**
 * 관리자 메뉴 공개방법별 현황 스크립트 파일이다
 *
 * @author 최성빈
...
function doAction(sAction) {
	...
	switch(sAction) {                       
		case "search":
			...
			sheet.DoSearchPaging(com.wise.help.url("/admin/expose/statAplOpbFom.do"), { Param : $("form[name=aplOpbForm]").serialize() });
			break;
		case "excel":
			sheet.Down2Excel({FileName:'공개방법별 현황.xls',SheetName:'sheet'});
			break;
...
function fn_print() {
	...
	formPrint.find("input[name=title]").val("공개방법별 현황");
	...
	formPrint.attr("action", com.wise.help.url("/admin/expose/infoPrintPage/printNewStatOpbFom.do"));
...
function goOpnAplListPop(instCd, saveNm) {
	...
	$("input[name=opbFomVAl]").val("");
	$("input[name=giveMth]").val("");
	$("input[name=opbY]").val("");
	 
	var form = $("form[name=aplOpbForm]");
	
	window.open('','popup', "width=800, height=800, scrollbars=yes");
	 
    $("input[name=aplDealInstCd]").val(instCd);
    $("input[name=opbY]").val("2"); 
    
    //SaveName 따라 파라미터 세팅
	if(saveNm == "totSum2"){
		$("input[name=giveMth]").val("00");
	}else if(saveNm == "sum1"){
		$("input[name=opbFomVAl]").val("01");
	}else if(saveNm == "sum2"){
		$("input[name=opbFomVAl]").val("02");
	}else if(saveNm == "sum3"){
		$("input[name=opbFomVAl]").val("03");
	}else if(saveNm == "sum4"){
		$("input[name=opbFomVAl]").val("04");
	}else if(saveNm == "sum5"){
		$("input[name=opbFomVAl]").val("05");
	}else if(saveNm == "totSum3"){
		$("input[name=giveMth]").val("00");
	}else if(saveNm == "sum6"){
		$("input[name=giveMth]").val("01");
	}else if(saveNm == "sum7"){
		$("input[name=giveMth]").val("02");
	}else if(saveNm == "sum8"){
		$("input[name=giveMth]").val("03");
	}else if(saveNm == "sum9"){
		$("input[name=giveMth]").val("04");
	}else if(saveNm == "sum10"){
		$("input[name=giveMth]").val("05");
	}
	...
	with(sheet){
	   ...
	   var headers = [
	    ...
	    var cols = [
	                ...
	                ,{Type:"AutoSum",	SaveName:"totSum2",	Width:150,	Align:"Center",		Edit:false,	Format : "Integer"}
					...
					,{Type:"AutoSum",	SaveName:"totSum3",	Width:150,	Align:"Center",		Edit:false,	Format : "Integer"}
					,{Type:"AutoSum",	SaveName:"sum6",	Width:150,	Align:"Center",		Edit:false,	Format : "Integer"}
					,{Type:"AutoSum",	SaveName:"sum7",	Width:150,	Align:"Center",		Edit:false,	Format : "Integer"}
					,{Type:"AutoSum",	SaveName:"sum8",	Width:150,	Align:"Center",		Edit:false,	Format : "Integer"}
					,{Type:"AutoSum",	SaveName:"sum9",	Width:150,	Align:"Center",		Edit:false,	Format : "Integer"}
					,{Type:"AutoSum",	SaveName:"sum10",	Width:150,	Align:"Center",		Edit:false, Format:  "Integer"}
