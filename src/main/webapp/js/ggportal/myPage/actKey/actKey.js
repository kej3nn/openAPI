...
var templates = {
		...
		, data_m : "<li>"
				...
				+"	</dl>"
				+"</li>"
		, none : "<tr><td colspan=\"6\" class=\"noData\">해당 자료가 없습니다.</td></tr>"
...
function bindEvent() {
	// 탭에 클릭 이벤트 바인딩
	$('#tab_layout a').each(function(i, d) {
...
//	if(!$('#tab_layout a:eq('+tabIndex+')').hasClass('on')) {
	if(!$('#tab_layout a#tab_'+tabIndex).hasClass('on')) {
		// 레이아웃 내용을 비운다.
		$('#content_'+tabIndex).empty();
		
		$('#tab_layout a').removeClass('on');
		$('div[id^=content_]').css('display', 'none');
		
		$('#tab_layout a#tab_'+tabIndex).addClass('on');
...
function afterSearchActKey(data) {
	$('#actkey-list').empty();
	$('#actkey-list-mob').empty();
	
	if(data.length > 0) {
		$('.totalNum').text(data[0].totalCnt);
		$.each(data, function(i, d) {
			var row = $(templates.data);
			var row_m = $(templates.data_m);
			
			row.find("td:eq(0)").text(d.actKey).prepend("<input type='radio' name='delActKey' value=\""+d.actKey+"\" title='삭제'/>  ");
			...
			row_m.find("strong").text(d.actKey).prepend("<input type='radio' name='delActKey' value=\""+d.actKey+"\" title='삭제'/>  "); 
		...
		$('#actkey-list-mob').empty().append($(templates.none_m));
	}
}
