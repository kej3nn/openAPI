/*
 * @(#)selectVisual.js 1.0 2015/06/15
 * 
 * COPYRIGHT (C) 2013 WISEITECH CO., LTD. ALL RIGHTS RESERVED.
 */

/**
 * 공공데이터 시각화 서비스를 조회하는 스크립트이다.
 *
 * @author 김은삼
...
var templates = {
    data:
        "<li class=\"w_2\">"                                                                            +
            "<span class=\"link vistnExp vistnUrl\"></span>"                                         +
            " "                                                                                         +
            "<span class=\"btn\">"                                                                      +
                "<a href=\"#\" class=\"btn_D visual-select-button\" title=\"새창으로 이동\">바로가기</a>" +
            "</span>"                                                                                   +
        "</li>",
    none:
 ...
 * 공공데이터 시각화 서비스 메타정보를 조회한다.
 */
function selectVisualMeta() {
    // 데이터를 조회한다.
    doSelect({
        url:"/portal/data/visual/selectOpenInfVisualMeta.do",
        before:beforeSelectVisualMeta,
        after:afterSelectVisualMeta
    });
}
 ...
 */
function searchVisualData() {
    // 데이터를 검색한다.
    doSearch({
        url:"/portal/data/visual/searchOpenInfVisualData.do",
        before:beforeSearchVisualData,
        after:afterSearchVisualData
    });
}
 ...
 */
function selectVisualData(data) {
    // 공공데이터 시각화 서비스 썸네일을 조회한다.
    selectVisualTmnl(data);
    
    // // 윈도우를 띄운다.
    // openWindow("/portal/data/visual/selectVisualData.do", "selectVisualData", {
    //     width:"600px",
    //     height:"400px"
    // }, {
    //     type:"object",
    ...
    // });
    if (data.vistnUrl) {
        var url = com.wise.help.url("/portal/data/visual/selectOpenInfVisualData.do");
        
        url += "?infId="   + data.infId;
        url += "&infSeq="  + data.infSeq;
        url += "&vistnSeq=" + data.vistnSeq;
 ...
 * @param data {Object} 데이터
 */
function selectVisualTmnl(data) {
    $("#visual-data-list li.on").each(function(index, element) {
        $(this).removeClass("on");
    });
    
    $("#visual-data-list .vistnExp.vistnUrl[href='#" + data.vistnSeq + "']").parent("li").addClass("on");
    
    if (data.tmnlImgFile) {
        var url = com.wise.help.url("/portal/data/visual/selectVisualTmnl.do");
        ...
        $("#visual-thumbnail-sect").html("<img src=\"" + com.wise.help.url(url) + "\" alt=\"\" />");
    }
    else {
        $("#visual-thumbnail-sect").empty();
    }
}
...
////////////////////////////////////////////////////////////////////////////////
/**
 * 공공데이터 시각화 서비스 메타정보 조회 전처리를 실행한다.
 * 
 * @param options {Object} 옵션
 * @returns {Object} 데이터
 */
function beforeSelectVisualMeta(options) {
    var data = {
        // Nothing do do.
    };
...
/**
 * 공공데이터 시각화 서비스 데이터 검색 전처리를 실행한다.
 * 
 * @param options {Object} 옵션
 * @returns {Object} 데이터
 */
function beforeSearchVisualData(options) {
    var data = {
        // Nothing do do.
    };
...
////////////////////////////////////////////////////////////////////////////////
/**
 * 공공데이터 시각화 서비스 메타정보 조회 후처리를 실행한다.
 * 
 * @param data {Object} 데이터
 */
function afterSelectVisualMeta(data) {
    // 공공데이터 시각화 서비스 데이터를 검색한다.
    searchVisualData();
}

/**
 * 공공데이터 시각화 서비스 데이터 검색 후처리를 실행한다.
 * 
 * @param data {Array} 데이터
 */
function afterSearchVisualData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        item.find(".vistnExp.vistnUrl").attr("href", "#" + data[i].vistnSeq).text(data[i].vistnExp + " - " + data[i].vistnUrl + " - " + data[i].prdNm + " - " + data[i].vistnTyNm);
        
        // 공공데이터 시각화 서비스 설명 링크에 클릭 이벤트를 바인딩한다.
        item.find(".vistnExp.vistnUrl").bind("click", {
            seq:data[i].seq,
            infId:data[i].infId,
            infSeq:data[i].infSeq,
            vistnSeq:data[i].vistnSeq,
            vistnUrl:data[i].vistnUrl,
            tmnlImgFile:data[i].tmnlImgFile
        }, function(event) {
            // // 공공데이터 시각화 서비스 썸네일을 조회한다.
             selectVisualTmnl(event.data);
            // 공공데이터 시각화 서비스 데이터를 조회한다.
            //selectVisualData(event.data);
            return false;
        });
        
        // 공공데이터 시각화 서비스 설명 링크에 키다운 이벤트를 바인딩한다.
        item.find(".vistnExp.vistnUrl").bind("keydown", {
            seq:data[i].seq,
            infId:data[i].infId,
            infSeq:data[i].infSeq,
            vistnSeq:data[i].vistnSeq,
        ...
        // 공공데이터 시각화 서비스 바로가기 버튼에 클릭 이벤트를 바인딩한다.
        item.find(".visual-select-button").bind("click", {
            seq:data[i].seq,
            infId:data[i].infId,
            infSeq:data[i].infSeq,
            vistnSeq:data[i].vistnSeq,
        ...
        // 공공데이터 시각화 서비스 바로가기 버튼에 키다운 이벤트를 바인딩한다.
        item.find(".visual-select-button").bind("keydown", {
            seq:data[i].seq,
            infId:data[i].infId,
            infSeq:data[i].infSeq,
            vistnSeq:data[i].vistnSeq,
    ...
    if (data.length == 0) {
        ...
        // $(".vistnExp.vistnUrl:first").click();
        // 공공데이터 시각화 서비스 썸네일을 조회한다.
        selectVisualTmnl({
            seq:data[0].seq,
            infId:data[0].infId,
            infSeq:data[0].infSeq,
            vistnSeq:data[0].vistnSeq,
