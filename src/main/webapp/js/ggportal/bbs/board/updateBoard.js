/*
 * @(#)updateBoard.js 1.0 2015/06/15
 * 
 * COPYRIGHT (C) 2013 WISEITECH CO., LTD. ALL RIGHTS RESERVED.
 */

/**
 * 일반 게시판 내용을 수정하는 스크립트이다.
 *
 * @author 김은삼
...
function bindMask() {
    ...
    $("#board-update-form [name=userTel1]").setMask({ mask:"999" });
    
    // 일반 게시판 내용 연락처 필드에 숫자 마스크를 바인딩한다.
    $("#board-update-form [name=userTel2]").setMask({ mask:"9999" });
    
    // 일반 게시판 내용 연락처 필드에 숫자 마스크를 바인딩한다.
    $("#board-update-form [name=userTel3]").setMask({ mask:"9999" });
}
 ...
 */
function bindEvent() {
    // 일반 게시판 내용 수정 폼에 제출 이벤트를 바인딩한다.
    $("#board-update-form").bind("submit", function(event) {
        return false;
    });
    
    // 일반 게시판 내용 이메일 콤보박스에 변경 이벤트를 바인딩한다.
    $("#board-update-form [name=userEmail3]").bind("change", function(event) {
        // 사용자 이메일을 변경한다.
    ...
    $("#board-update-button").bind("click", function(event) {
        // 일반 게시판 내용을 수정한다.
        updateBoard();
        return false;
    });
    ...
    $("#board-update-button").bind("keydown", function(event) {
        if (event.which == 13) {
            // 일반 게시판 내용을 수정한다.
            updateBoard();
            return false;
...
function changeUserEmail(value) {
    if (value == "na") {
        $("#board-update-form [name=userEmail2]").prop("readonly", false);
    }
    else {
        $("#board-update-form [name=userEmail2]").prop("readonly", true).val(value);
    }
}
...
/**
 * 일반 게시판 내용 수정 전처리를 실행한다.
 * 
 * @param options {Object} 옵션
 * @returns {Object} 데이터
 */
function beforeUpdateBoard(options) {
    var form = $("#board-update-form");
    ...
    //     return false;
    // }
    if(!com.wise.util.isBlank(form.find("[name=newBulletIdInfo]").val())) {
    	form.find("[name=newBulletIdInfo]").val(encryptByDES(form.find("[name=newBulletIdInfo]").val(), bulletIdInfoKey));
    }
...
function afterSelectBoard(data) {
    for (var key in data) {
        ...
        $("#board-update-form").find("." + key).each(function(index, element) {
    ...
    if (data.lockTag == 'P') {
        $("#board-update-form [name=newBulletIdInfo]").parent("td").parent("tr").removeClass("hide");
    }
}

/**
 * 일반 게시판 내용 수정 후처리를 실행한다.
 * 
 * @param messages {Object} 메시지
 */
function afterUpdateBoard(messages) {
    // 일반 게시판 내용을 취소한다.
    cancelBoard();
}
