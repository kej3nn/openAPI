...
OpenTab.prototype ={
		...
		addProcess : function(id,title,url,callback){    
			...
			if ($("a[id="+this.TabName+"]").length == 0) {//탭이 존재하는지 확인    
				$(".all").after(this.TabTemplate.replace( /#\{href\}/g,this.TabName ).replace( /#\{label\}/g, XSSfilter(title) ) );
		...
		addTabClickEvent: function(){     //탭 이벤트 추가   
			...
			var tabALen = 0; //tabId.find("a[name=tab-a]").length -1;
			if(tabALen > 1){ 
				tabALen = 1;           
			}
			tabId.find("a[name=tab-a]").eq(tabALen).click(function(e) {
				...
				if($(this).parent().parent().attr("class") != "other_list"){  //숨김탭이 아니면
					if($(this).parent().attr("class") !="all"){ //전체목록제외
			...
			tabId.find(".tab_close").eq(0).click(function(e) {      
				...
				if($(this).parent().parent().attr("class") != "other_list"){ //다른 탭이 아니면                          
					...
					if($(this).parent().prev().attr("class") !="all"){ //전체목록제외
		...
		selectTabContent:function (url,callback){   //ajax db 조회    
			...
			//var _data = new FormData($('#commonForm')[0]);
			var tabObj = this.TabObj;
			$.ajax({                          
				url:url,
				processData: false,
		...
		,tabModalOpen : function(){
			var html = "<div class='abcd' style='display:none;position: absolute; left: 20px; top: 420px; width: 900px; height: 500px; display: block;z-index: 264;background:#FFFFFF;opacity: 0.5;filter: alpha(opacity=50)'>&nbsp;</div>";
