...
<tr>
	...
	<td style='text-align:center;border:1px solid #d3d3d3;'>
		<c:if test="${result.gubunTag eq 'T' }">
			<a href='http://${serverNm}<c:if test="${serverPort ne '' }">:${serverPort }</c:if>/portal/infs/cont/infsContPage.do?infsId=${result.infsId}&cateId=${result.parInfsId}'>정보셋 보기</a>
		</c:if>
	</td>
