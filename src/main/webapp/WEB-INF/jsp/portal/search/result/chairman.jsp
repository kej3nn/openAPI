...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
		...
		<section class="ysearch_section">
                    <div class="ysearch_result_tit">
                        <div class="ysearch_result_tit_txt">${chairman.colDisplayName}(<span><fmt:formatNumber value="${chairman.totalCount}" pattern="#,###"/>건</span>)</div>
                        <fmt:parseNumber var="cnt" type="number" value="${chairman.totalCount }"  />
                       <c:if test="${fn:contains(vo.collection, 'ALLA') &&  cnt >3  }"> 
                        <a href="javascript:doCollection('${chairman.colIndexName}' , 'assemT')">더보기 <span>+</span></a>
                       </c:if>
                    </div>
                     <c:forEach var="data" items="${chairman.srchList }">
                    <article>
