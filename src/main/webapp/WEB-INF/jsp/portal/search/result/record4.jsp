...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
		<section class="ysearch_section">
                    <div class="ysearch_result_tit">
                        <div class="ysearch_result_tit_txt">${record4.colDisplayName}(<span><fmt:formatNumber value="${record4.totalCount}" pattern="#,###"/>건</span>)</div>
                       <c:if test="${fn:contains(vo.collection, 'ALL') }">   
                        <a href="javascript:doCollection('${record4.colIndexName}' , 'recordTab')">더보기 <span>+</span></a>
                       </c:if>
                    </div>
                     <c:forEach var="data" items="${record4.srchList }">
                     <article>
