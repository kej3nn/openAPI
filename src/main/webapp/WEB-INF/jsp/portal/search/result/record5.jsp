...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
		<section class="ysearch_section">
                    <div class="ysearch_result_tit">
                        <div class="ysearch_result_tit_txt">${record5.colDisplayName}(<span><fmt:formatNumber value="${record5.totalCount}" pattern="#,###"/>건</span>)</div>
                       <c:if test="${fn:contains(vo.collection, 'ALL') }">   
                        <a href="javascript:doCollection('${record5.colIndexName}' , 'recordTab')">더보기 <span>+</span></a>
                       </c:if>
                    </div>
                     <c:forEach var="data" items="${record5.srchList }">
                    <article>
                       <!-- <div class="ysearch_result_01">사전정보공개 &gt; 국회의원 &gt; 역대 국회의원</div> --> 
                        <div class="ysearch_result_02">
                            <a href="http://likms.assembly.go.kr/record/mhs-10-050.do?conferNum=${data.CONFER_NUM }&pdfFileId=${data.PDF_ID}&subSpage=P" target="_blank">${fn:replace( fn:replace(data.TAG , HS , HS_VAL) , HE , HE_VAL)}</a>
                            <span class="ysearch_result_02_date"> (${data.RDATE })</span>
                        </div>
                        
                         <a href="http://likms.assembly.go.kr/record/mhs-10-050.do?conferNum=${data.CONFER_NUM }&pdfFileId=${data.PDF_ID}&subSpage=P" target="_blank" class="ysearch_result_03">
