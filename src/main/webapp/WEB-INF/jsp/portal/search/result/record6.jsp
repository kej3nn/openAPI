...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
		<section class="ysearch_section">
                    <div class="ysearch_result_tit">
                        <div class="ysearch_result_tit_txt">${record6.colDisplayName}(<span><fmt:formatNumber value="${record6.totalCount}" pattern="#,###"/>건</span>)</div>
                       <c:if test="${fn:contains(vo.collection, 'ALL') }">   
                        <a href="javascript:doCollection('${record6.colIndexName}' , 'recordTab')">더보기 <span>+</span></a>
                       </c:if>
                    </div>
                     <c:forEach var="data" items="${record6.srchList }">
                     <article>
