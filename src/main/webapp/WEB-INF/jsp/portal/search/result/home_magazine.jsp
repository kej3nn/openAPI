...
<%@ taglib prefix="ui" uri="http://egovframework.gov/ctl/ui" %>
		...
		<section class="ysearch_section">
                    <%-- <div class="ysearch_result_tit">
                        <div class="ysearch_result_tit_txt">${home_magazine.colDisplayName}(<span><fmt:formatNumber value="${home_magazine.totalCount}" pattern="#,###"/>건</span>)</div>
                        <fmt:parseNumber value="${home_magazine.totalCount }" var="cnt" />
                       <c:if test="${fn:contains(vo.collection, 'ALLA')  && cnt >3}">   
                        <a href="javascript:doCollection('${home_magazine.colIndexName}' , 'assemT')">더보기 <span>+</span></a>
                       </c:if>
                    </div> --%>
                     <c:forEach var="data" items="${home_magazine.srchList }">
                    <article>
