...
<form name="adminOpenInfOptPop"  method="post" action="#">
			...
			<div class="popup">
					...
					<div style="padding:15px;">
						...
						<c:set var="icalSeq" value="${item.colSeq}" />
						<c:set var="ocalSeq" value="${openInfSrv.colSeq}" />
						<tr>
							<td><input type="radio" name="newColId" value="${item.colId}" <c:if test="${icalSeq eq ocalSeq}">checked=checked</c:if> />
								&nbsp;<c:out value="${item.colId}"/>
							</td>
							<td><c:out value="${item.colNm}"/></td>
						</tr>
						</c:forEach>
