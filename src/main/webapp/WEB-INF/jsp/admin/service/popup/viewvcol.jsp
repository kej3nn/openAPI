...
function inputSet() {
	var formObj = $("form[name=OpenInfVColView]");
...
<div class="wrap-popup">
		...
		<div class="container">
			...
			<!-- 탭 내용 -->                 
			<form name="OpenInfVColView"  method="post" action="#">             
			<div class="content-popup">
				...
				<tr>
					<c:choose>
						<c:when test="${viewLang eq 'E'}">
							<th class="none">No</th>
							<th>Visualize URL</th>
							<th>Visualize URL Name</th>
							<th>Maker</th>
							<th>Visualize Name</th>
							<th>View Count</th>
							<th>Regist Day</th>
							<th>Shot cut</th>
						</c:when>
				...
				<tr>
					...
					<td>${resultList.vistnUrl }</td>
					<c:choose>
					<c:when test="${viewLang eq 'E'}">
						<td>${resultList.vistnExpEng }</td>
					</c:when>
					<c:otherwise>
						<td>${resultList.vistnExp }</td>
					</c:otherwise>
					</c:choose>
					<td class="text-center">${resultList.prdNm }</td>
					<td class="text-center">${resultList.vistnTyNm }</td>
					<td class="text-center">${resultList.viewCnt }</td>
					<td class="text-center">${resultList.regDttm }</td>
					<td class="text-center">
						<c:choose>
							<c:when test="${viewLang eq 'E'}">
								<button type="button" class="btn01L" id="btn_link" onclick="javascript: goUrl('${resultList.vistnUrl}');">Shot cut▶</button>
							</c:when>
							<c:otherwise>         
								<button type="button" class="btn01L" id="btn_link" onclick="javascript: goUrl('${resultList.vistnUrl}');">바로가기▶</button>
							</c:otherwise>        
						</c:choose>
					</td>
				</tr>
