...
function mySheet1_OnDblClick(row, col, value, cellx, celly) {
    if(row == 0) return;   
    //alert(mySheet1.GetCellValue(row, "infNm"));
    tabEvent(row);                      
...
function tabEvent(row){//탭 이벤트 실행     
    ...
    //var js = '{"infId":"OOWY4R001216HX11226"}';
    //openTab.SetTabData(JSON.parse(js));
    var url = "<c:url value='/admin/service/openInfSrvtInfo.do'/>"; // Controller 호출 url        
    //console.log(mySheet1.GetRowJson(row));
    //console.log(js);
...
function tabEventByInfId(title, id){//탭 이벤트 실행  
	//alert(id);
	//var objTab = getTabShowObj();
	//openTab.removeTab(objTab);
	openTab.removeShowTab();
	var title = title;//탭 제목                                                    
	var id = id;//탭 id(유일한key))
    //openTab.SetTabData(mySheet1.GetRowJson(row));//db data 조회시 조건 data
    var js = '{"infId":"'+id+'"}';
    openTab.SetTabData(JSON.parse(js));
    var url = "<c:url value='/admin/service/openInfSrvtInfo.do'/>"; // Controller 호출 url        
...
function onClick(Row, Col, Value, CellX, CellY, CellW, CellH) {
	...
	//objTabDiv.find("input[name=srvCd]").val()
			...
			case "colId":
				if(status =="I"){
					...
					if(srvCd=='S') {
						wWidth ="660";                                                    
						wHeight ="600";                         
						url = "<c:url value='/admin/service/openInfColIdPopUp.do"+data+"'/>";
						openIframePop(iframeNm,url,wWidth,wHeight);//ifrmNm, url,width,height
					}
				}
				break; 
	...
	if(json.DATA != null){//아따 겁다 짜증나...
		$.each(json.DATA,function(key,state){ 
			...
			$.each(state,function(key2,state2){ 
				...
				if(key2 =="ownerCd" || key2 =="dsId" || key2 =="dsNm" || key2 =="infNm"|| key2 =="infId" || key2 =="openInfIds"){
					
					// 2023-11, add api version check.
					if(key2=="openInfIds") {
						strOpenInfIds = state2;
					} else {
		...
		// 2023-11, api version check.. item = > infId
		if(strOpenInfIds != "") {
			infIdArr = strOpenInfIds.split("|");
			$.each(infIdArr, function (index, item){
				if(strInfId == item)
					tab.ContentObj.find("td[id=sinfo]").append("<input type='radio' name='apiVersion' value='"+item+"' onClick='tabEventByInfId(\""+strInfNm+"\", \""+item+"\");' checked=checked> <b>V"+(++index)+"</b>&nbsp;&nbsp;");
				else
					tab.ContentObj.find("td[id=sinfo]").append("<input type='radio' name='apiVersion' value='"+item+"' onClick='tabEventByInfId(\""+strInfNm+"\", \""+item+"\");' > <b>V"+(++index)+"</b>&nbsp;&nbsp;");
			})	
		}
	}
