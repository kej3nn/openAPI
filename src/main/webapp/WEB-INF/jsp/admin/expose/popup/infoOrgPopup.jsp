<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>
<%@ page import="egovframework.common.WiseOpenConfig"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="ui" uri="http://egovframework.gov/ctl/ui"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags"%>
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>  
<%@ taglib prefix="validator" uri="http://www.springmodules.org/tags/commons-validator" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title><spring:message code='wiseopen.title'/></title>                  
<c:import  url="/WEB-INF/jsp/inc/headinclude.jsp"/>
</head>                                                
<style type="text/css">
body{background:none;}                                     
</style>                  
<script language="javascript">                
var popObj = "form[name=form]";
 
$(document).ready(function()    {
	bindEvent();
	loadPage();
	$("input[name=searchVal]").focus();
});
 
function bindEvent() {
	$(popObj).find("a[name=a_close]").click(function(e) { 
		parent.closeIframePop("iframePopUp");
		return false;                
	});
	
	$(popObj).find(".popup_close").click(function(e) {             
		$(popObj).find("a[name=a_close]").click();
		return false;                
	}); 
	
	$(popObj).find("button[name=btn_search]").click(function(e) { 
		doAction("search");                
		return false;                
	});
	
	$(popObj).find("input").keypress(function(e) {                   
		  if(e.which == 13) {
			  doAction('search');   
			  return false; 
		  }
	});
	
	// 추가
	$(popObj).find("a[name=a_save]").bind("click", function(e) {
		addCate();
		return false;
	})
 }

function addCate() {

	var parentFormNm = $("#parentFormNm").val();
	var parentObj = window.parent.$("form[name="+ parentFormNm +"]");
	
 	var strChkRows = sheet.FindCheckedRow("orgNm");
 	if ( gfn_isNull(strChkRows) ) {
 		alert("저장하려는 부서명을 선택하세요.");
 		return false;
 	}
 	
	var chkRows = strChkRows.split('|');
	var arrAplDeptCd = [];
	var arrAplDeptNm = [];
	
	for ( var i=0; i < chkRows.length; i++ ) {
		arrAplDeptCd.push(sheet.GetCellValue(chkRows[i], "orgCd"));
		arrAplDeptNm.push(sheet.GetCellValue(chkRows[i], "orgNm"));
	}

	if ( arrAplDeptCd.length > 0 ) {
		parentObj.find("[name=aplDeptCd]").val(arrAplDeptCd[0]);
		parentObj.find("[name=aplDeptCds]").val(arrAplDeptCd.join(","));
		parentObj.find("[name=aplDeptNm]").val(arrAplDeptNm.join(","));
	} 
	else {
		parentObj.find("[name=aplDeptCd]").val("");
		parentObj.find("[name=aplDeptCds]").val("");
		parentObj.find("[name=aplDeptNm]").val("");
	}
	
	parent.closeIframePop("iframePopUp");
	parent.doAction("deptSave");
}

//시트초기화
function loadPage() {
	
	createIBSheet2(document.getElementById("sheet"),"sheet", "100%", "300px");
	
	var gridTitle ="NO";
		gridTitle +="|부서코드";
		gridTitle +="|부서명";
		gridTitle +="|최상위부서코드";
		
    with(sheet){
    	                     
    	var cfg = {SearchMode:2, Page:50, VScrollMode:1, TreeNodeIcon:2};
        SetConfig(cfg);
        var headers = [
                    {Text:gridTitle, Align:"Center"}
                ];
        var headerInfo = {Sort:0, ColMove:0, ColResize:1};
        
        InitHeaders(headers, headerInfo);
        var cols = [ 
					 {Type:"Seq",		SaveName:"seq",				Width:30,	Align:"Center",		Edit:false}	
					,{Type:"Text",		SaveName:"orgCd",			Width:50,	Align:"Center",		Edit:false}
					,{Type:"Text",		SaveName:"orgNm",			Width:300,	Align:"Left",		Edit:false,	TreeCol:1, TreeCheck:1, CheckSaveName: "chk"}
					,{Type:"Text",		SaveName:"topOrgCd",		Width:50,	Align:"Center",		Edit:false, Hidden:true}

               ];    
        InitColumns(cols);
        FitColWidth();
        SetTreeCheckActionMode(0);
        SetExtendLastCol(1);
    }               
    default_sheet(sheet);   
    
    doAction("search");
} 

//action 함수
function doAction(sAction) {
	
	var formObj = $("form[name=form]");  
	
	switch(sAction) {
		case "search":      //조회   
			ajaxBeforeSendAdmin(com.wise.help.url("/admin/ajaxBeforeSendAdmin.do")); //IbSheet 조회전 세션 체크
			var param = {PageParam: "ibpage", Param: "onepagerow=50"+"&"+formObj.serialize()};
			sheet.DoSearchPaging(com.wise.help.url("/admin/expose/popup/infoOrgPopList.do"), param);
			break;
	}
}
     
//시트 더블클릭 선택시
/* 
function sheet_OnDblClick(row, col, value, cellx, celly) {
    if(row == 0) return;  
	var arrayValue;
	
	var topRow = 0;
   	var parRow = sheet.GetParentRow(row);
   	if ( parRow > 0 ) {
   		do {
   			topRow = parRow;
   			parRow = sheet.GetParentRow(topRow);
   		} while ( parRow > 0 )	
   	} else {
   		topRow = row;
   	}

    arrayValue = {
    		aplDeptCd : sheet.GetCellValue(row, "orgCd"), 
    		aplDeptNm : sheet.GetCellValue(row, "orgNm")
   	};
	
	// 부모 폼으로 arrayValue에 있는 값을 넘겨준다.
	var parentFormNm = $("#parentFormNm").val();
	var parentObj = window.parent.$("form[name="+ parentFormNm +"]");
		           
	$.each(arrayValue,function(key,state){
		parentObj.find("[name="+key+"]").val(state);
	}); 
	
	parent.closeIframePop("iframePopUp");
   
} 
 */
function sheet_OnSearchEnd(code, message, statusCode, statusMessage) {
	 doAjax({
			url : "/admin/expose/selectOpnzDeptList.do",
			params : "aplNo=" + $("#aplNo").val(),
			callback : function(res) {
				var orgCd = "";
				var data = res.data;
				
				var parentChkcked = $("#aplDeptCds").val().split(",");
				if ( !gfn_isNull($("#aplDeptCds").val())  ) {
					for ( var r in parentChkcked ) {
						data.push({ aplNo: $("#aplNo").val(), orgCd: parentChkcked[r] });
					}
				}
				for ( var r in data ) {
					orgCd = data[r].orgCd;
					for ( var i=0; i < sheet.RowCount(); i++ ) {
						if ( orgCd == sheet.GetCellValue(i, "orgCd") ) {
							sheet.SetTreeCheckValue(i, 1);
						}
					}
				}
			}
		});
}
</script>                
<body>
<form name="form"  method="post" action="#">
	<input type="hidden" id="parentFormNm" value="${param.parentFormNm}"/>
	<input type="hidden" id="aInstCd" name="aInstCd" value="${param.aInstCd}"/>
	<input type="hidden" id="aplDeptCds" name="aplDeptCds" value="${param.aplDeptCds}"/>
	<input type="hidden" id="aplNo" name="aplNo" value="${param.aplNo}"/>
	<div class="popup">
		<h3>부서선택 팝업</h3>
		<a href="#" class="popup_close">x</a>
		<div style="padding:15px;">
			<table class="list01">              
			<colgroup>
				<col width="150"/>
				<col width=""/>
			</colgroup>
				<tr>
					<th>검색어</th>
					<td>
						<select id="searchGubun" name="searchGubun">
							<option value="ORG_NM">부서명</option>
						</select>
						<input type="text" id="searchVal" name="searchVal" value="" width="70%"/>
					</td>
				</tr>
			</table>
			<div align="right">
				<button type="button" class="btn01" title="검색" name="btn_search">검색</button>
			</div>
			
			<div class="ibsheet_area" id="sheet">
			</div>
			
			<div class="buttons">
				${sessionScope.button.a_save}
				${sessionScope.button.a_close}
			</div>         
		</div>
	</div>
</form>
</body>
</html>                             