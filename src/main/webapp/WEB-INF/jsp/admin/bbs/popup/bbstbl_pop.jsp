...
function doAction(sAction)                                  
{
	...
	{          
		case "search":      //조회   
			...
			popSheet.DoSearchPaging("<c:url value='/admin/bbs/popup/selectSttsTblPopList.do'/>", param);
			break;
		case "add" :
			addTbl("R");
			break;
...
function addTbl(tblLinkTag) {
	var parSheet = opener.parent.getTblSheet();		// 부모창 시트
	...
	for ( var i=0; i < chkRows.length; i++ ) {
		...
		parSheet.SetCellValue(newRow, "statblId", popSheet.GetCellValue(chkRows[i], "statblId"));
		parSheet.SetCellValue(newRow, "statblNm", popSheet.GetCellValue(chkRows[i], "statblNm"));
		parSheet.SetCellValue(newRow, "bbsCd", $("#bbsCd").val());
		parSheet.SetCellValue(newRow, "seq", $("#seq").val());
		parSheet.SetCellValue(newRow, "tblLinkTag", tblLinkTag);
	}
	
	// 중복된 통계표 ID 확인하여 제거함
	while ( parSheet.ColValueDup("statblId") > -1 ) {
		parSheet.RowDelete(parSheet.ColValueDup("statblId"));
	}
