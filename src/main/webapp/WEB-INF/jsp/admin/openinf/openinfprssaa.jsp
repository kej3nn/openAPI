...
function tabPrssCallBack(tab,json){
	tab.ContentObj.find("a[name=prssCA]").hide();
	if(json.DATA != null){
...
// 					tab.ContentObj.find("a[name=prssAA]").show();
// 					tab.ContentObj.find("a[name=prssCA]").hide();
// 				}
				if(key2 == "prssState" && state2 == "AO"){	//등록중(개방요청 전)
// 					tab.ContentObj.find("a[name=prssAA]").hide();
					//tab.ContentObj.find("a[name=prssCA]").show();
					setPrssAccCdBtn(tab, "prssAA", prssAccCd);	//개방요청 버튼 show/hide
				} 
				if(key2 == "prssState" && state2 == "AA"){	//개방 요청상태
					setPrssAccCdBtn(tab, "prssCA", prssAccCd);	//개방 취소 버튼 show/hide
				}
				if(key2 == "apiRes"){
					if(state2 != null){
...
// 	var url = "<c:url value='/admin/openinf/openLogInfPrssExpDtl.do'/>";
// 	var objTab = getTabShowObj();//탭이 oepn된 객체가져옴
// 	var formObj = tab.ContentObj.find("form[name=adminOpenInfPrss]");        
// 	var param = formObj.serialize();
// 	ajaxCall(url, param, openLogInfCallBack);    
	
	setTabButton();
}

/**
 * 권한에 따라 objNm 버튼 표시(권한은 승인(40)이상 있을 경우 버튼 표시)
 */
function setPrssAccCdBtn(tab, objNm, prssAccCd) {
	if ( prssAccCd >= 40 ) {
		tab.ContentObj.find("a[name="+objNm+"]").show();
	} 
	else {
		tab.ContentObj.find("a[name="+objNm+"]").hide();
	}
}

// function openLogInfCallBack(tab, json, res){
// 	var objTab = getTabShowObj();//탭이 oepn된 객체가져옴
// 	var formObj =objTab.find("form[name=adminOpenInfPrss]");        
// 	if ( tab.result != null ) {
// // 		alert(tab.result[0].prssExp);  
// 		formObj.find("[name=prssExp]").val(tab.result[0].prssExp);
// 	}
// }
...
function setTabButton(){		//버튼event
	...
	formObj.find("a[name=prssAA]").click(function() {		// 개방요청
 		doTabAction("prssAA");          
 		return false;                 
 	});
	
	formObj.find("a[name=prssCA]").click(function() {		// 개방요청취소
 		doTabAction("prssCA");          
 		return false;                 
 	});
...
function doTabAction(sAction){	
	...
	switch(sAction){
		...
		case "prssAA":
 			if ( !confirm("개방요청 하시겠습니까? ") ) {
				return;
  			}
			if(!saveCheck(formObj)){	return false;	}
...
function saveCheck(formObj){
	if(formObj.find("input[name=openDttm]").val() == ""){
		alert("개방(예정)일자를 입력해주세요.");
		return false;
	}
	return true;
}
			...
			<div class="content" style="display:none">
				...
				<table class="list01">
					...
					<tr>
						...
						<td colspan="3">
							<div style="float:left;"><label for="infNm"><input type="text" name="infNm" size="60" class="text-read" readonly/></label></div> 
							<div style="float:right;"><button type="button" class="btn01L" name="datasetView">데이터셋상세</button>
...
<!-- 						<td colspan="3"> -->
<%-- 							<input type="checkbox" name="korYn" value="Y" disabled/> <label class=""><spring:message code="labal.kor"/></label> --%>
<%-- 							<input type="checkbox" name="engYn" value="Y" disabled/> <label class=""><spring:message code="labal.eng"/></label> --%>
<%-- 							<input type="checkbox" name="korMobileYn" value="Y" disabled/> <label class=""><spring:message code="labal.mobile"/></label> --%>
<!-- 						</td> -->
<!-- 					</tr> -->
					<tr>
					...
					<tr>
						...
						<td colspan="3">
							...
							<input type="text" name="orgNm" size="60" class="text-read" readonly/>
						</td>
					</tr>
					<tr>
						...
						<th>연락처</th>
						<td>
							<input type="text"  name="tel" class="text-read"  readonly/>
						</td>
					</tr>
					<tr>
						...
						<th>개방(예정)일자</th>
						<td>
							<input type="text" name="openDttm" readonly="readonly"/>
						</td>
					</tr>
					<tr>
						<td colspan="4"></td>
				...
				<div class="buttons">
					<a href="#" class="btn03" name="prssAA" style="display: none;">개방요청</a>					
					<a href="#" class="btn03" name="prssCA" style="display: none;">개방요청취소</a>
