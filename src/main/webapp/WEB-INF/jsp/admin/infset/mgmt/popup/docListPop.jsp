...
function LoadSheet() {
    ...
    with(sheet){
	    ...
	    var cols = [
			...
			, {Type:"Text",		SaveName:"docNm",			Width:200,	Align:"Left",		Edit:false,	Hidden:false}
 ...
 */
function add() {
	var parentSheetObj = parent.getTabSheetObj("mst-form", "docSheetNm");	// 부모창의 문서데이터 탭 시트 객체
	
 	var strChkRows = sheet.FindCheckedRow("check");	// 체크된 행
 	if ( gfn_isNull(strChkRows) ) {
 		alert("추가하려는 문서를 선택하세요.");
 		return false;
 	}
	...
	for ( var i=0; i < chkRows.length; i++ ) {
		...
		parentSheetObj.SetCellValue(newRow, "docId", 	sheet.GetCellValue(chkRows[i], "docId"));
		
		// ID 중복체크하여 중복되면 입력하려고 추가한행 제거
		if ( parentSheetObj.ColValueDup("docId") >= 0 ) {
			parentSheetObj.RowDelete(newRow);
			continue;
		}
		parentSheetObj.SetCellValue(newRow, "docNm",	sheet.GetCellValue(chkRows[i], "docNm"));
...
function doAction(sAction) {
	...
	switch(sAction) {
		case "search" :
			sheet.DoSearch("/admin/infs/mgmt/popup/selectDocListPop.do", formObj.serialize());
			break;
	}
}
</script>                
				...
				<tr>
					...
					<td>
						<select id="searchGubun" name="searchGubun">
							<option value="DOC_NM">문서명</option>
							<option value="DOC_ID">문서ID</option>
						</select>
