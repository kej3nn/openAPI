...
function LoadSheet() {
    ...
    with(sheet){
	    ...
	    var cols = [
            ...
            , {Type:"Text",		SaveName:"infId",			Width:100,	Align:"Center",		Edit:false,	Hidden:false}
			...
			, {Type:"Text",		SaveName:"orgNm",			Width:190,	Align:"Left",		Edit:false,	Hidden:false}
...
function setSheetOptions() {
	doAjax({
		...
		callback: function(res) {
			...
			sheet.SetColProperty(0, "infState", options);
		}
	});
}
 ...
 */
function add() {
	var parentSheetObj = parent.getTabSheetObj("mst-form", "openSheetNm");	// 부모창의 공공데이터 탭 시트 객체
	...
	for ( var i=0; i < chkRows.length; i++ ) {
		...
		parentSheetObj.SetCellValue(newRow, "infId", 	sheet.GetCellValue(chkRows[i], "infId"));
		
		// ID 중복체크하여 중복되면 입력하려고 추가한행 제거
		if ( parentSheetObj.ColValueDup("infId") >= 0 ) {
			parentSheetObj.RowDelete(newRow);
			continue;
		}
		parentSheetObj.SetCellValue(newRow, "infNm", 	sheet.GetCellValue(chkRows[i], "infNm"));
		...
		parentSheetObj.SetCellValue(newRow, "infState", sheet.GetCellValue(chkRows[i], "infState"));
...
function doAction(sAction) {
	...
	switch(sAction) {
		case "search" :
			sheet.DoSearch("/admin/infs/mgmt/popup/selectOpenListPop.do", formObj.serialize());
			break;
	}
}
</script>                
				...
				<tr>
					...
					<td>
						<select id="searchGubun" name="searchGubun">
							<option value="INF_NM">공공데이터명</option>
							<option value="INF_ID">공공데이터ID</option>
						</select>
