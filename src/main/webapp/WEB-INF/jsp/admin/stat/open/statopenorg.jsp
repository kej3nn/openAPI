...
{      
	...
	var gridTitle = "NO";
		gridTitle +="|"+"기관명";		// 기관명
		gridTitle +="|"+"서비스";	// 서비스 수
		gridTitle +="|"+"Sheet";		// Sheet 수
		//gridTitle +="|"+"<spring:message code='labal.T_CNT'/>";		// TS Sheet 수
		gridTitle +="|"+"Chart";		// Chart 수
		gridTitle +="|"+"Map";		// Map 수
		gridTitle +="|"+"File";		// File 수
		gridTitle +="|"+"Link";		// Link 수
		gridTitle +="|"+"Open API";		// Open API수 
		gridTitle +="|"+"시각화";		// 멀티미디어 수
//		gridTitle +="|"+"<spring:message code='labal.VN_CNT'/>";		// 시각화 수
...
function doAction(sAction)                                  
{
	...
	{          
		case "search":      //조회
			...
			mySheet.DoSearchPaging("<c:url value='/admin/stat/open/getOpenStatOrgSheetAll.do'/>", param);   
			
			//차트 조회
			ajaxCallAdmin("<c:url value='/admin/stat/open/getOpenStatOrgChartAll.do'/>",formObj.serialize(),initializeChart);
...
function initializeChart(data){    
	...
	rytitNm = "건수";
	//X축 스타일                   
	initXaxisChart(obj,varColor, varColor, "기관");//chartObj, x색, y색, 이름
	// 왼쪽 Y축 스타일
	initYaxisChart(obj,varColor, varColor, "건수");//chartObj, x색, y색, 이름
	// 왼쪽 Y축 스타일2                              
	initRYaxisChart(obj,varColor, varColor, "");//chartObj, x색, y색, 이름
	//툴팁 스타일
	initToolTipStyle(obj);//chartObj               
	//범례 스타일                               
	initLegendStyle(obj,"center","bottom","horizontal","N");//chartObj, 가로, 세로, 정렬, 플로팅
	//Label 스타일
	initLableStyle(obj);//chartObj                          
	//chart 데이터 추가         
