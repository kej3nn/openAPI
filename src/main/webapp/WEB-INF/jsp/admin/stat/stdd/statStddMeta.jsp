...
<script type="text/javascript" src="<c:url value="/js/admin/stat/statStddMeta.js" />"></script>
...
<body onload="">
	<div class="wrap">
		...
		<div class="container">
			...
			<c:import  url="statStddMetaDtl.jsp"/>
			...
			<div class="content">
				...
				<table class="list01">
					...
					<tr>
						<th>통계구분</th>
						<td colspan="3">
							<c:forEach var="sttsCd" items="${sttsCdList }" varStatus="status">
								<c:choose>
									<c:when test="${status.first }">
										<input type="radio" name="searchSttsCd" id="sttsCd${sttsCd.code }" value="${sttsCd.code }" checked="checked">
											<label for="sttsCd${sttsCd.code }">${sttsCd.name }</label>
										</input>
									</c:when>
									<c:otherwise>
										<input type="radio" name="searchSttsCd" id="sttsCd${sttsCd.code }" value="${sttsCd.code }">
											<label for="sttsCd${sttsCd.code }">${sttsCd.name }</label>
										</input>									
									</c:otherwise>
								</c:choose>
					...
					<tr>
						...
						<td colspan="3">
							<select name="searchGubun" value="">
								<option value="META_NM">메타항목명</option>
								<option value="ENG_META_NM">영문메타항목명</option>
								<option value="META_ID">메타ID</option>
							</select>
