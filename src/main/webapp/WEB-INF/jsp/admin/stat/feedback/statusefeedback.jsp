...
function setDate(){
		var formObj = $("form[name=statOpen]");
		var nowTo = new Date();	
		var nowFrom = new Date(nowTo-(3600000*24*7));	
...
{      
    ...
    with(mySheet){
        ...
        var cols = [ 
					...
					,{Type:"AutoSum",		SaveName:"userCnt",			Width:80,	Align:"Right",		Edit:false}
					,{Type:"AutoSum",		SaveName:"keyCnt",			Width:80,	Align:"Right",		Edit:false}
					,{Type:"AutoSum",		SaveName:"reCnt",			Width:80,	Align:"Right",		Edit:false}
					,{Type:"AutoSum",		SaveName:"qaCnt",			Width:80,	Align:"Right",		Edit:false}
					,{Type:"AutoSum",		SaveName:"gaCnt",			Width:80,	Align:"Right",		Edit:false}
                   ];       
...
function doAction(sAction)                                  
{
	...
	{          
		case "search":      //조회
			...
			mySheet.DoSearchPaging("<c:url value='/admin/stat/feedback/getUseFBStatSheetAll.do'/>", param);   
			
			//차트 조회
			ajaxCallAdmin("<c:url value='/admin/stat/feedback/getUseFBStatChartAll.do'/>",formObj.serialize(),initializeChart);
	...
	for(var i=0; i < data.seriesResult.length ; i++){    
       ...
       /*  //Y축 column 이름 셋팅
		...
		var series = myChart.CreateSeries();  
       	if(YcolNm[i] == "USER_CNT"){	// 전체활용 수는 꺽은선 그래프로 표현
