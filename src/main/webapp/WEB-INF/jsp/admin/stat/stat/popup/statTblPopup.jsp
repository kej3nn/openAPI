...
var popObj = "form[name=statTblPopup]";
...
//시트초기화
function loadStatTblSheet() {
	
	createIBSheet2(document.getElementById("statTblPopSheet"),"statTblPopSheet", "100%", "300px");
    ...
    with(statTblPopSheet){
        ...
        var cols = [
					...
					,{Type:"Text",		SaveName:"engStatblNm",		Width:160,	Align:"Left",		Edit:false}
                ];                                          
        InitColumns(cols);
        FitColWidth();
        SetExtendLastCol(1);
    }               
    default_sheet(statTblPopSheet);   
    
    doAction("search");
} 

// 시트 더블클릭시
function statTblPopSheet_OnDblClick(row, col, value, cellx, celly) {
	if(row == 0) return;  
	var arrayValue;
	
	arrayValue = {
			statblId : statTblPopSheet.GetCellValue(row, "statblId"),
			statblNm : statTblPopSheet.GetCellValue(row, "statblNm")
	}
...
function doAction(sAction) {
	...
	var formObj = $("form[name=statTblPopup]");  
	
	switch(sAction) {
		case "search" :
			ajaxBeforeSendAdmin("<c:url value='/admin/ajaxBeforeSendAdmin.do'/>"); //IbSheet 조회전 세션 체크
			var param = {PageParam: "ibpage", Param: "onepagerow=50"+"&"+formObj.serialize()};
			statTblPopSheet.DoSearchPaging("<c:url value='/admin/stat/popup/statTblPopupList.do'/>", param);
			break;
	}
}
...
<body>
<form name="statTblPopup"  method="post" action="#">
	...
	<div class="popup">
		...
		<div style="padding:15px;">
				...
				<tr>
					...
					<td>
						<select id="searchGubun" name="searchGubun">
							<option value="STATBL_NM">한글통계명</option>
							<option value="ENG_STATBL_NM">영문통계명</option>
						</select>
			...
			<div class="ibsheet_area" id="statTblPopSheet">
			</div>
